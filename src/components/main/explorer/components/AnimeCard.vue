<template>
  <router-link :to="`/explorer/anime/${anime.id}`">
    <div
      class="flex flex-col h-full bg-white rounded-xl overflow-hidden border border-gray-200 transition-colors group"
    >
      <div class="relative pb-[130%] bg-gray-100 overflow-hidden">
        <img
          ...
          :alt="anime.title"
          class="absolute top-0 left-0 w-full h-full object-cover rounded-t-2xl transition-transform duration-500 ease-in-out transform group-hover:scale-110"
        />
        <div
          v-else
      ...
      <div class="p-4 flex flex-col flex-grow">
        ...
        <div class="flex gap-2">
          <span
            ...
            :key="index"
            class="text-xs px-3 py-1 bg-blue-100 text-blue-800 rounded-md font-medium"
          >
            {{ genre }}
          </span>
        </div>
