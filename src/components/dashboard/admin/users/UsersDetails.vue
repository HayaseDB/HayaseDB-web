<template>
  ...
  <div
    class="p-6 relative overflow-hidden bg-white rounded-2xl max-w-2xl mx-auto space-y-6 border border-gray-200"
  >
      ...
      >
        ...
        <tbody class="bg-white divide-y divide-gray-100">
          ...
          >
            ...
            >
              <span
                v-if="field.label === 'Role'"
                :class="roleTagClasses(field.value)"
              >
                {{ field.value }}
              </span>
              <span v-else>
...
async function fetchUser() {
  loading.value = true;
  try {
    const data = await UserService.getUserProfile(userId.value);
    user.value = data;
...
const userFields = computed(() => {
  ...
  return [
    ...
    {
      label: "Verified",
      value: user.value.verified ? "Yes" : "No",
      isStatus: true,
    },
    {
      label: "Banned",
      value: user.value.banned ? "Yes" : "No",
      isStatus: true,
    },
...
async function toggleVerification(verify: boolean) {
  try {
    if (verify) {
      const response = await UserService.verifyUser(userId.value);
      toast.success(response.message);
    } else {
      const response = await UserService.unverifyUser(userId.value);
      toast.success(response.message);
    }
    await fetchUser();
  } catch (error) {
...
async function toggleBan() {
  try {
    if (!isBanned.value) {
      const response = await UserService.banUser(userId.value);
      toast.success(response.message);
    } else {
      const response = await UserService.unbanUser(userId.value);
      toast.success(response.message);
    }
    await fetchUser();
  } catch (error) {
